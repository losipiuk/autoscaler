...
func TestGetMigNodesBasic(t *testing.T) {
	...
	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-9j4g", nodes[0].GceRef.ToProviderId())
	assert.Nil(t, nodes[0].ErrorInfo)
	assert.Equal(t, InstanceRunning, nodes[0].Status)

	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-c63g", nodes[1].GceRef.ToProviderId())
	assert.Nil(t, nodes[1].ErrorInfo)
	assert.Equal(t, InstanceRunning, nodes[1].Status)

	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-dck1", nodes[2].GceRef.ToProviderId())
	assert.Nil(t, nodes[2].ErrorInfo)
	assert.Equal(t, InstanceRunning, nodes[2].Status)

	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-f1hm", nodes[3].GceRef.ToProviderId())
	assert.Nil(t, nodes[3].ErrorInfo)
	assert.Equal(t, InstanceRunning, nodes[3].Status)
	mock.AssertExpectationsForObjects(t, server)
}
...
func TestGetMigNodesComplex(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedErrorClass   InstanceErrorClass
		...
		},
		{
			...
			buildManagedInstanceWithCurrentActionAndErrorResponsePart("europe-west1-b", "instance-creating-quota-exceeded", "CREATING", "QUOTA_EXCEEDED", "We run out of quota while creating!"),
			InstanceCreating,
		...
		},
		{
			...
			buildManagedInstanceWithCurrentActionAndErrorResponsePart("europe-west1-b", "instance-recreating-quota-exceeded", "RECREATING", "QUOTA_EXCEEDED", "We run out of quota while recreating!"),
			InstanceCreating,
			QuotaExceededErrorClass,
		...
		},
		{
			...
			buildManagedInstanceWithCurrentActionAndErrorResponsePart("europe-west1-b", "instance-creating-no-retries-quota-exceeded", "CREATING_WITHOUT_RETRIES", "QUOTA_EXCEEDED", "We run out of quota while creating without retries!"),
			InstanceCreating,
			QuotaExceededErrorClass,
		...
		},
		{
			...
			buildManagedInstanceWithCurrentActionAndTwoErrorsResponsePart("europe-west1-b", "instance-creating-other-error-and-quota-exceeded", "CREATING", "SOME_ERROR", "Ojojojoj!", "QUOTA_EXCEEDED", "We run out of quota!"),
			InstanceCreating,
			QuotaExceededErrorClass,
		...
		},
		{
			...
			buildManagedInstanceWithCurrentActionAndTwoErrorsResponsePart("europe-west1-b", "instance-creating-quota-exceeded-and-other", "CREATING", "QUOTA_EXCEEDED", "We run out of quota!", "SOME_ERROR", "Ojojojoj!"),
			InstanceCreating,
			QuotaExceededErrorClass,
	...
	for i, tc := range testCases {
		...
		assert.Equal(t, fmt.Sprintf("gce://project1/europe-west1-b/%s", tc.instanceName), instanceInfo.GceRef.ToProviderId())
		assert.Equal(t, tc.expectedStatus, instanceInfo.Status)
		if tc.expectedErrorClass == 0 {
			assert.Nil(t, instanceInfo.ErrorInfo)
		} else {
			assert.NotNil(t, instanceInfo.ErrorInfo)
			assert.Equal(t, tc.expectedErrorClass, instanceInfo.ErrorInfo.ErrorClass)
			assert.Equal(t, tc.expectedErrorMessage, instanceInfo.ErrorInfo.ErrorMessage)
		}
	}
...
func buildListInstanceGroupsResponse(zone string, instanceGroups ...string) string {
	...
	for _, instanceGroup := range instanceGroups {
		...
		items += buildInstanceGroupResponsePart(zone, instanceGroup)
	}
