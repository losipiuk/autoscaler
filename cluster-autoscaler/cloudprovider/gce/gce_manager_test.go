...
func TestGetMigNodesBasic(t *testing.T) {
	...
	assert.Equal(t, NONE, nodes[0].ErrorClass)
	...
	assert.Equal(t, NONE, nodes[1].ErrorClass)
	assert.Equal(t, RUNNING, nodes[1].Status)

	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-dck1", nodes[2].GceRef.ToProviderId())
	assert.Equal(t, NONE, nodes[2].ErrorClass)
	assert.Equal(t, RUNNING, nodes[2].Status)

	assert.Equal(t, "gce://project1/us-central1-b/gke-cluster-1-default-pool-f7607aac-f1hm", nodes[3].GceRef.ToProviderId())
	assert.Equal(t, NONE, nodes[3].ErrorClass)
	assert.Equal(t, RUNNING, nodes[3].Status)
	mock.AssertExpectationsForObjects(t, server)
}
...
func TestGetMigNodesComplex(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		assert.Equal(t, tc.expectedErrorClass, instanceInfo.ErrorClass)
		assert.Equal(t, tc.expectedStatus, instanceInfo.Status)
		assert.Equal(t, tc.expectedErrorMessage, instanceInfo.ErrorMessage)
	}
